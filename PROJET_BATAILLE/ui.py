...
Module : ui.py
Description : Interface graphique (GUI) gérant l'affichage
des grilles et des boutons.
...
class BatailleUI:
    ...
    Cette classe crée la fenêtre de jeu, dessine les deux grilles
    (joueur et adversaire)
    avec des étiquettes de coordonnées (A-J, 1-10) et gère
    les mises à jour visuelles.
    
    Attributes:
        root (tk.Tk): La fenêtre principale de l'application.
        boutons_ma_flotte (dict): Dictionnaire stockant les boutons
        de la grille joueur.
        bateau_adversaire (dict): Dictionnaire stockant les boutons
        de la grille adverse.
    """

    def __init__(self, root: tk.Tk, callback_tir: CALLBACKCASE,
                 callback_placement: CALLBACKCASE) -> None:
        """
        Initialise l'interface, configure les couleurs de fond et
        créer les grilles.

        Args:
            root (tk.Tk): Fenêtre parente.
            callback_tir (CALLBACKCASE): Fonction à appeler lors d'un tir.
            callback_placement (CALLBACKCASE): Fonction à appeler
            lors du placement.
        """
        self.root = root
        ...
        self.creer_grille_complete("ATTAQUE ADVERSAIRE", tk.RIGHT,
                                   self.bateau_adversaire, callback_tir,
                                   "#ECEFF1")
        
    def creer_grille_complete(self, titre: str, cote: int, dico: dict,
                              callback: CALLBACKCASE, bg_color: str):
        """
        Construit une grille 10x10 avec des labels pour les colonnes (A-J)
        et les lignes (1-10).

        Args:
            titre (str): Nom affiché au-dessus de la grille.
            cote (int): Positionnement (tk.LEFT ou tk.RIGHT).
            dico (dict): Dictionnaire pour stocker les références
            des boutons créés.
        ...
        for c in range(10):
            tk.Label(grid_frame, text=self.lettres[c], width=4, bg="#37474F",
                     fg="white",
                     font=("Arial", 10, "bold")).grid(row=0, column=c+1)
    ...
    def colorier(self, grille: GRILLE, ligne: int, colonne: int,
                 couleur: str) -> None:
        """
        Met à jour l'apparence d'une case (couleur et symbole) selon le résultat
        du jeu.

        Args:
            grille (GRILLE_TYPE): "moi" pour la flotte du joueur, "adversaire"
            pour l'attaque.
            ligne (int): Index de la ligne (1-10).
            colonne (int): Index de la colonne (1-10).
            couleur (str): Identifiant de l'état
        ...
        if (ligne, colonne) in target:
            if couleur == "red":
                ...
                target[(ligne, colonne)].config(bg="#FF5252", text="X",
                                                fg="white")
            elif couleur == "black":
                # Tir raté : Gris-bleu avec un ~ (vague)
                target[(ligne, colonne)].config(bg="#546E7A", text="~",
                                                fg="white")
            elif couleur == "blue":
                # Bateau placé : Bleu
