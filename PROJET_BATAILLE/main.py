...
class Controle:
    ...
    def config_initiale(self):
        """permet de chosir le mode et de connecter au besoin selon si l'ordi est hôte ou serveur"""
        print("\n" + "="*40)
        ...
        if choix: # c'est comme if choix == True
    ...
    def on_place(self, ligne, colonne):
       """permet de placer les bateaux si le jeu n'a pas commencé (que je suis pas prêt) puis avertit qu'ilest prêt""" 
        if self.pret_moi:
            return
    ...
    def verifier_demarrage(self):
        """vérifie qui est en train de jouer pour afficher le bon message"""
    ...
    def on_tir(self, ligne, colonne):
        """vérifie si tout le monde a placé ses bateaux et si c'est mon tour , puis si mode solo : """
        ...
        if self.mode == "SOLO":
            ...
            self.mon_tour = False
            self.root.after(800, self.faire_jouer_ia)
